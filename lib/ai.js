...
// ------------------------------------------------------------
//  4) GENERATOR NASKAH VIDEO (Timed Script for Video Studio)
// ------------------------------------------------------------
export async function generateVideoScript({ listing, scenes }) {
  if (!aiEnabled()) {
    return { script: fallbackVideoScript(listing, scenes), aiUsed: false };
  }

  const sys =
    "Anda adalah copywriter video iklan properti (social media ads creator) top 1% di Indonesia. " +
    "Anda sangat memahami konsep video hook, retention rate, dan CTA yang menjual. " +
    "Tugas Anda adalah menulis naskah suara (voiceover script) berupa 1 kalimat pendek per adegan video. " +
    "Gunakan bahasa Indonesia yang semi-formal, persuasif, menarik, dan enak didengar saat dibacakan. " +
    "PENTING ATURAN PELAFALAN ELEVENLABS: " +
    "1) DILARANG KERAS menggunakan singkatan seperti LT, LB, KT, KM, m², atau m2. Selalu tulis lengkap menjadi kata utuh: 'luas tanah', 'luas bangunan', 'kamar tidur', 'kamar mandi', 'meter persegi'. " +
    "2) Jangan gunakan angka numerik (seperti '120' atau '3'), melainkan eja angkanya menjadi kata-kata (seperti 'seratus dua puluh' atau 'tiga') agar suara ElevenLabs membacanya mengalir tanpa terputus-putus. " +
    "3) Pada adegan harga ('price'), sebutkan harganya DAN sebutkan ukuran luas tanah serta kamarnya secara mengalir, jelas, dan tidak terputus. " +
    "Balas HANYA dengan JSON valid sesuai skema, tanpa teks lain.";

  const scenesStr = scenes.map((s, idx) => `Adegan ${idx}: tipe: ${s.kind}, info/caption tambahan: ${s.caption || ""}`).join("\n");

  const prompt =
    `DATA PROPERTI:\n` +
    `- Judul: ${listing.title}\n` +
    `- Tipe: ${TYPE_LABELS[listing.type] || listing.type}\n` +
    `- Status: ${listing.listing === "sewa" ? "Disewakan" : "Dijual"}\n` +
    `- Lokasi: ${listing.location}\n` +
    `- Harga: ${formatPrice(listing.price, listing.listing, listing.priceUnit)}\n` +
    `- Spesifikasi: ${listing.bedrooms} KT, ${listing.bathrooms} KM, LT ${listing.landSize}m², LB ${listing.buildingSize}m²\n` +
    `- Keunggulan: ${(listing.sellingPoints || []).map(sp => `${sp.aspect}: ${sp.point}`).join("; ")}\n\n` +
    `DAFTAR ADEGAN VIDEO:\n${scenesStr}\n\n` +
    `TUGAS:\n` +
    `Tulis naskah suara (voiceover) berupa tepat 1 kalimat pendek per adegan. Naskah harus pendek agar mudah dibaca cepat (sekitar 8-15 kata per adegan). ` +
    `Buat hook yang luar biasa kuat untuk adegan 'hook' (misalnya dengan pertanyaan retoris atau fakta unik). ` +
    `Pastikan semua angka dan singkatan ukuran dieja penuh menjadi huruf.\n\n` +
    `Balas dengan JSON berformat persis:\n` +
    `{\n` +
    `  "script": [\n` +
    `    { "kind": "hook", "text": "kalimat naskah adegan hook..." },\n` +
    `    { "kind": "price", "text": "kalimat naskah adegan harga..." },\n` +
    `    ...\n` +
    `  ]\n` +
    `}`;

  try {
    const text = await ask({ tier: "smart", system: sys, content: prompt, maxTokens: 1500 });
    const data = parseJson(text);
    if (!data || !Array.isArray(data.script) || data.script.length !== scenes.length) {
      return { script: fallbackVideoScript(listing, scenes), aiUsed: false };
    }
    return { script: data.script, aiUsed: true };
  } catch (err) {
    console.error("generateVideoScript AI error:", err?.message || err);
    return { script: fallbackVideoScript(listing, scenes), aiUsed: false, error: String(err?.message || err) };
  }
}

function fallbackVideoScript(listing, scenes) {
  const priceStr = formatPrice(listing.price, listing.listing, listing.priceUnit);
  return scenes.map((s) => {
    let text = "";
    if (s.kind === "hook") {
      text = `Mencari hunian mewah impian di ${listing.location?.split(",")[0] || "lokasi terbaik"}?`;
    } else if (s.kind === "price") {
      const specs = [];
      if (listing.landSize) specs.push(`luas tanah ${listing.landSize} meter persegi`);
      if (listing.buildingSize) specs.push(`luas bangunan ${listing.buildingSize} meter persegi`);
      if (listing.bedrooms) specs.push(`${listing.bedrooms} kamar tidur`);
      if (listing.bathrooms) specs.push(`${listing.bathrooms} kamar mandi`);
      const specStr = specs.length ? ` dengan ${specs.join(", ")}` : "";
      text = `Rumah ini ditawarkan seharga ${priceStr.replace(/[^0-9a-zA-Z\s]/g, "")} rupiah${specStr}.`;
    } else if (s.kind === "photo") {
      text = s.caption ? `${s.caption}.` : "Perhatikan desain ruangan yang sangat lapang dan modern ini.";
    } else if (s.kind === "point") {
      text = `${s.sp?.point || "Keunggulan utama hunian ini"} untuk kenyamanan hidup Anda.`;
    } else if (s.kind === "nearby") {
      text = `Lokasi strategis dekat fasilitas utama, pusat perbelanjaan, dan akses jalan tol.`;
    } else if (s.kind === "cta") {
      text = "Jadwalkan kunjungan Anda sekarang juga, hubungi kontak kami di bawah ini.";
    }
    return { kind: s.kind, text: text || "Lihat detail properti menakjubkan ini." };
  });
}
