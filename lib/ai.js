...
export async function ask({ tier = "smart", system, content, maxTokens = 1500 }) {
...
export async function generateListing({ details = {}, images = [] }) {
  ...
  try {
    ...
    const text = await ask({ tier: "smart", system: sys, content, maxTokens: 4000 });
...
// ------------------------------------------------------------
export async function generateLocationInsight({ location, type, listing }) {
  if (!location) return { ...fallbackInsight(location), aiUsed: false };
  if (!aiEnabled()) return { ...fallbackInsight(location), aiUsed: false };

  try {
    // Butuh pengetahuan kawasan nyata (risiko halusinasi) → model smart.
    const text = await ask({
      tier: "smart",
      maxTokens: 1200,
      system:
        "Anda analis kawasan properti Indonesia yang mengenal kota, kawasan, dan cluster besar. " +
        "Berikan analisis lokasi yang KONKRET (nama tempat nyata di sekitar lokasi itu). Bila tidak " +
        "yakin nama persisnya, sebut kategorinya secara umum tanpa mengarang nama. Balas HANYA JSON valid.",
      content:
            `Lokasi properti: "${location}" (tipe: ${type || "rumah"}, status: ${listing === "sewa" ? "disewakan" : "dijual"}).\n\n` +
            "Balas JSON persis:\n" +
            `{
  "summary": "1-2 kalimat: posisi strategis lokasi ini dalam peta kota/kawasan",
  "strategic": ["3-5 alasan strategis yang spesifik (akses tol/transport, arah pengembangan, pusat ekonomi, banjir/kontur bila relevan)"],
  "nearby": [ { "name": "nama tempat nyata terdekat", "category": "Tol|Sekolah|Rumah Sakit|Mall|Transportasi|Kuliner|Kampus|Perkantoran", "minutes": 5 } ]
}\n` +
            "nearby: 6-8 tempat, urutkan dari paling dekat, minutes = estimasi berkendara.",
    });
    const data = parseJson(text);
    if (!data || !Array.isArray(data.nearby)) return { ...fallbackInsight(location), aiUsed: false };
    return {
      summary: data.summary || "",
      strategic: Array.isArray(data.strategic) ? data.strategic : [],
      nearby: data.nearby.filter((n) => n?.name).slice(0, 8),
      aiUsed: true,
    };
  } catch (err) {
    console.error("generateLocationInsight AI error:", err?.message || err);
    return { ...fallbackInsight(location), aiUsed: false };
  }
}
...
export async function generateArticle({ topic, audience }) {
  ...
  try {
    ...
    const text = await ask({ tier: "smart", system: sys, content: prompt, maxTokens: 6000 });
    const data = parseJson(text);
