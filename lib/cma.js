...
export function findComparables(subject, pool, limit = 6) {
  return pool
    .filter((l) => l.slug !== subject.slug && l.price > 0 && refArea(l) > 0 && l.listing === subject.listing)
    .map((l) => ({ l, score: similarity(subject, l) }))
    .filter((x) => x.score >= 3) // minimal tipe cocok / lokasi menyerempet
  ...
  try {
    const text = await ask({
      ...
      system:
        ...
        "dengan metode sales comparison. Gunakan angka pembanding yang diberikan + pengetahuan kawasan. " +
        "Jujur, berbasis data, siap dipakai memenangkan mandat jual dari pemilik. Balas HANYA JSON valid.",
      content:
        `SUBJEK:\n${subLine}\n\nPEMBANDING (dari katalog):\n${compLines}\n\n` +
