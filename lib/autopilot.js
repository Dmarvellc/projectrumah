...
import { saveListing, addJob, getSettings, updateSettings, getBrand, brandAgent } from "@/lib/store";
...
export function parseSpecOffline(raw) {
  ...
  const garage = toNumber(labeled(text, "garasi")) || 0;
  const iplRaw = labeled(text, "ipl|iuran");
  const ipl = iplRaw ? parseRupiah(iplRaw) : 0;
  const FACILITY_SCAN = [
    ["kolam renang", "Kolam Renang"], ["taman", "Taman"], ["gudang", "Gudang"],
    ["kitchen set", "Kitchen Set"], ["water heater", "Water Heater"], ["cctv", "CCTV"],
    ["smart home", "Smart Home"], ["one gate", "One Gate System"], ["solar panel", "Solar Panel"], [" ac ", "AC"],
  ];
  const facilities = FACILITY_SCAN.filter(([k]) => lower.includes(k)).map(([, v]) => v);
...
export async function runAutoListing(rawSpec, opts = {}) {
  const { publish = true, marketing: withMarketing = true, images: photos = [], galleryUrls = [] } = opts;
  const stages = [];
  ...
  // b2. Titik peta + analisis kawasan (strategis & tempat terdekat NYATA)
  const locationFull = [details.cluster, details.location].filter(Boolean).join(", ");
  const geo = await geocode(locationFull || details.location);
  const locationInsight = await generateLocationInsight({
    location: locationFull || details.location,
    type: details.type,
  ...
  // c. Aset visual — utamakan URL galeri yang sudah diunggah klien (galleryUrls);
  //    fallback: base64 photos → simpan server-side; terakhir: foto stok.
  const urls = galleryUrls.length ? galleryUrls : photos.length ? await saveImagesToUploads(photos) : [];
  let images;
  let photoCaptions = content.photoCaptions || [];
  ...
  if (urls.length) {
    ...
    // caption HARUS mengikuti urutan foto (cover pindah ke depan)
    if (photoCaptions.length) {
      photoCaptions = [photoCaptions[idx], ...photoCaptions.filter((_, i) => i !== idx)].filter((c) => c !== undefined);
    }
  } else {
    // TANPA foto upload → TANPA foto sama sekali. Jangan pernah pakai foto
    // stok/palsu — menyesatkan pembeli. UI menampilkan blok "Foto menyusul".
    images = [];
    photoCaptions = [];
    if (photos.length) {
      // Foto DIKIRIM tapi gagal disimpan — jangan diam: beri tahu kenapa.
      photoWarning = process.env.VERCEL && !process.env.BLOB_READ_WRITE_TOKEN
        ? "Foto Anda TIDAK terpakai: Vercel Blob belum terhubung (Storage → Blob → Connect, lalu redeploy). Listing sementara memakai foto stok."
        : "Foto Anda gagal disimpan — listing sementara memakai foto stok. Coba jalankan ulang.";
      stages.push({ name: "Foto upload gagal — pakai stok", status: "error" });
    }
  }
  let assembled = {
    ...details,
  ...
  const record = saveListing({
    ...
    maidRooms: Number(assembled.maidRooms) || 0,
    garage: Number(assembled.garage) || 0,
    ipl: Number(assembled.ipl) || 0,
    facilities: assembled.facilities || [],
...
export async function runAutoBatch(text, opts = {}) {
  const { images = [], galleryUrls = [] } = opts;
  const hasPhotos = images.length > 0 || galleryUrls.length > 0;
  const specs = hasPhotos ? [String(text).trim()] : splitSpecs(text);
  const results = [];
