// ============================================================
//  Content store — DATABASE (Vercel Blob) sebagai sumber tunggal.
//
//  Produksi: SELALU database (Blob). Tidak ada penyimpanan lokal /
//  /tmp ephemeral sama sekali — bila DB belum tersambung, data hidup
//  di memori sesaat dan banner memperingatkan (bukan diam-diam hilang
//  ke /tmp seperti dulu).
//
//  Dev lokal: memakai file content/db.json HANYA jika BLOB token tidak
//  diset. Isi BLOB_READ_WRITE_TOKEN di .env.local untuk memakai database
//  yang sama di lokal (nol file lokal).
// ============================================================
...
// File lokal hanya untuk pengembangan tanpa database (tidak pernah di produksi).
const LOCAL_DIR = path.join(process.cwd(), "content");
const LOCAL_FILE = path.join(LOCAL_DIR, "db.json");
const useLocalFile = !blobDbEnabled() && !process.env.VERCEL;
...
// Cache in-memory: sumber kebenaran runtime, disinkronkan dari database.
let memDb = null;

const hydrate = (raw) =>
  raw
    ? { ...structuredClone(EMPTY), ...raw, settings: { ...EMPTY.settings, ...(raw.settings || {}) } }
    : structuredClone(EMPTY);

// DATABASE: hidrasi SEKALI saat cold start (top-level await menjamin ini
// selesai sebelum handler mana pun berjalan). Data bertahan melewati redeploy.
if (blobDbEnabled()) {
  memDb = await loadDbFromBlob();
}

// Baca TIDAK PERNAH menulis/melempar.
export function readDb() {
  if (blobDbEnabled()) return hydrate(memDb);
  if (useLocalFile) {
    try {
      if (fs.existsSync(LOCAL_FILE)) return hydrate(JSON.parse(fs.readFileSync(LOCAL_FILE, "utf8")));
    } catch {}
  }
  return hydrate(memDb);
}

export function writeDb(db) {
  memDb = db;
  if (blobDbEnabled()) {
    // Write-through ke database; waitUntil menjaga penulisan selesai walau
    // respons HTTP sudah terkirim (serverless) → durabel.
  ...
  // Produksi tanpa database tersambung: memori saja (banner memperingatkan).
}

// Sinkron ulang memori dari database — dipakai sebelum mutasi agar
// read-modify-write berbasis data terbaru (hindari update saling menimpa
// antar-instance serverless).
export async function refreshDb() {
  if (!blobDbEnabled()) return;
  const fresh = await loadDbFromBlob();
  if (fresh) memDb = fresh;
}
